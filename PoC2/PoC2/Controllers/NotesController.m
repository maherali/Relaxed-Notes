...
- (id) initWithValues:(NSDictionary*) passedInValues{
    ...
    $watch(@"note:add", ^(NSNotification *notif) {
        ...
        $navigate($sprintf(@"/notes/%@", @""), $dict(MODEL_KEY, aNote));
    });
    [self refreshNotes];
    return self;
}
...
- (void) viewDidLoad{
    ...
    self.tableView = [[[NotesView alloc] initWithOptions:$dict(COLLECTION_KEY, collection, @"section_header_title", headerTitle) andStyle:UITableViewStylePlain] autorelease];
